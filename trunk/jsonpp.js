    ...
    var m = {
        ...
        s = {
            array: function (x) {
            	...
            	var nested = false;
            	for (i = 0; i < l; i += 1) if (
            		typeof x[i] == 'object' && x[i] != null
            		) {nested = true; break;}
                if (nested) {a = newline(a); intendLevel++;}
                for (i = 0; i < l; i += 1) {
                    v = x[i];
                    f = s[typeof v];
                    if (f) {
                        ...
                        if (typeof v == 'string') {
                            if (b) {
                                a[a.length] = ', ';
                                if (nested) a = newline(a);
                            }
                            if (nested) a = intend(a); // else a[a.length] = " ";
                            a[a.length] = v;
                            b = true;
                        }
                    }
                }
                if (nested) {intendLevel--; a = newline(a); a = intend(a);}
                a[a.length] = ']';
                return a.join('');
            },
            'boolean': function (x) {
